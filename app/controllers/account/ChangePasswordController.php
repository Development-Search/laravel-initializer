...
class ChangePasswordController extends AuthorizedController {
	...
	protected function postIndex()
	{
		...
		if ( ! $user->checkPassword(Input::get('old_password')))
		{
			...
			return Redirect::route('account.change-password.index')->withErrors($this->messageBag);
		}
		...
		// Redirect to the change-password page
		return Redirect::route('account.change-password.index')->with('success', 'Password successfully updated');
	}
