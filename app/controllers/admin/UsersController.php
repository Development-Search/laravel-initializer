...
class UsersController extends AdminController {
	...
	{
		$all_users = Sentry::getUserProvider()->createModel();

		$all_users_count           = $all_users->withTrashed()->count();
		$activated_users_count     = $all_users->where('activated', '=', 1)->count();
		$non_activated_users_count = $all_users->where('activated', '=', 0)->count();
		$deleted_users_count       = $all_users->onlyTrashed()->count();

		$users = $all_users->where('activated', '=', 1)->paginate(20);
		$data = compact('all_users_count', 'activated_users_count', 'non_activated_users_count', 'deleted_users_count', 'users');

		// Show the page
		return View::make('backend.users.index', $data);
	}

	public function getNonActivate()
	{
		$all_users = Sentry::getUserProvider()->createModel();
		...
		$users = $all_users->where('activated', '=', 0)->paginate(20);
		$data = compact('all_users_count', 'activated_users_count', 'non_activated_users_count', 'deleted_users_count', 'users');
	...
	public function getDeleted()
	{
		...
		$users = $all_users->onlyTrashed()->paginate(20);
		$data = compact('all_users_count', 'activated_users_count', 'non_activated_users_count', 'deleted_users_count', 'users');
	...
	public function getAll()
	{
		...
		// Paginate the users
		$users = $all_users->withTrashed()->paginate(20);
		$data = compact('all_users_count', 'activated_users_count', 'non_activated_users_count', 'deleted_users_count', 'users');
	...
	public function postCreate()
	{
		...
		try
		{
			...
			if ($user = Sentry::getUserProvider()->create($inputs))
			{
				...
				return Redirect::route('user.update.index', $user->id)->with('success', $success);
			}
			...
			return Redirect::route('user.create.index')->with('error', $error);
		}
		...
		// Redirect to the user creation page
		return Redirect::route('user.create.index')->withInput()->with('error', $error);
	}
	...
	public function postEdit($id = null)
	{
		...
		try
		{
			...
			if ($user->save())
			{
				...
				return Redirect::route('user.update.index', $id)->with('success', $success);
			}
		...
		// Redirect to the user page
		return Redirect::route('user.update.index', $id)->withInput()->with('error', $error);
	}
	...
	public function getDelete($id = null)
	{
		try
		{
			...
			{
				// Redirect to the user management page
				return Redirect::route('user.index')->with('error', 'Insufficient permissions!');
			}
